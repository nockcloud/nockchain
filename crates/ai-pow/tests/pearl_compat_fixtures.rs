...
//!
//!   * **S0** — Pearl protocol constants.
//!   * **S1** — `get_random_hash` byte stream.
//!   * **S2** — permutation pairs.
//!   * **S3** — uniform-noise rows.
//!   * **S4** — `matvec_sparse_perm` arithmetic.
//!   * **S5** — full tile loop / `jackpot[16]`.
//!   * **S6** — `compute_jackpot_hash`.
//!   * **S7** — commitment-hash chain.
//!   * **S8** — matrix-commitment chunk-Merkle root.
//!   * **S9** — shape-aware difficulty target.
//!
