Nockchain end-to-end scenarios live in `crates/nockchain-e2e/e2e/scenarios`.

Run a scenario locally:

```bash
cargo build -p nockchain --release
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/smoke.yaml
```

Two-node sync example:

```bash
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/two_nodes_sync.yaml
```

Additional scenarios:

```bash
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/upgrade_activation.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/mixed_version.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/partition_reorg.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/invalid_tx_rejected.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/wallet_smoke.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/transaction_lifecycle.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/double_spend_rejected.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_partition_reorg.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_multi_sender.yaml
```
...
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_shipped_default.yaml

# Gen2 send enabled on both nodes
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_enabled.yaml

# Mixed: gen2-enabled sender plus explicit accept-only peer
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_mixed_generation.yaml

# Restart/rejoin after gen2 session
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_rollback.yaml

# Full transaction lifecycle with gen2 enabled on all peers
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_tx_lifecycle.yaml

# Adversarial transaction coverage with gen2 enabled on all peers
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_double_spend.yaml
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_invalid_tx.yaml

# Fail-fast wallet/block-stuffer smoke before the long soak
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_block_stuffer_preflight.yaml
...
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_soak.yaml
...
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_rolling_restart.yaml

# Rolling per-node rollback from gen2 send to gen1
cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_gen2_rolling_rollback.yaml
...
NOCKCHAIN_BIN_OLD=/path/to/old NOCKCHAIN_BIN_NEW=target/release/nockchain \
  cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/nous_old_new_fallback.yaml
...
- Example: `cargo run -p nockchain-e2e -- run crates/nockchain-e2e/e2e/scenarios/smoke.yaml --docker --docker-image nockchain-e2e:ci`.
