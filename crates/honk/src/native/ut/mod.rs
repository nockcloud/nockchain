...
pub struct Ut<'a> {
    ...
    // to avoid recursive re-entry during `mull` checks.
    fire_wet_rib: Vec<(Noun, Noun, Noun)>,
    fire_wet_rib_raw: FastHashSet<(u64, u64, u64)>,
...
impl<'a> Ut<'a> {
    ...
    fn mint_fits(
        ...
        let formula = match &port {
            ...
            Port::Palo(palo) if matches!(palo.opal, Opal::Leg(_)) => {
                ...
                self.type_test_formula_on_axis(ref_type, axis)?
            }
            _ => {
                let (_ty, base_formula) = self.fine(&port)?;
                let test = self.type_test_formula_on_axis(ref_type, 1)?;
    ...
    fn mint_wthx(
        ...
        let (hit_ty, axis) = self.fend(sut, Way::Read, wing)?;
    ...
    fn type_test_formula_on_axis(&mut self, typ: Noun, axis: u64) -> Result<Noun> {
    ...
    fn type_test_formula_on_axis_inner(
        ...
        match tag.as_str() {
            "void" => Ok(const_bool_formula(self.slab, false)),
            "noun" => Ok(const_bool_formula(self.slab, true)),
            "atom" => {
                let (_aura, value) = type_atom_parts(typ, &self.slab.noun_space())?;
            ...
            "hold" => {
                for prior in seen_holds.iter() {
                    if unsafe { prior.raw_equals(&typ) }
                        || noun_eq(*prior, typ, &self.slab.noun_space())?
                    {
            ...
            _ => Err(CompilerError::Decode(format!(
                "fish unsupported type tag: {tag}"
            ))),
        }
    }
    ...
    fn skin_test_formula(&mut self, sut: Noun, axis: u64, skin: &Skin) -> Result<Noun> {
        ...
        match skin {
            ...
            Skin::Over(wing, inner) => {
                let rel_axis = self.resolve_wing_axis(sut, wing)?;
    ...
    fn type_test_formula_on_axis_fork(
        ...
        let head_formula = self.type_test_formula_on_axis_inner(*head, axis, seen_holds)?;
    ...
    fn cnts_tack(&mut self, sut: Noun, wing: &WingType, mur: Noun) -> Result<(u64, Noun)> {
        let port = self.find(sut, Way::Rite, wing)?;
        match port {
            Port::Palo(palo) => {
                let duz = |_ut: &mut Self, _a: Noun| Ok(mur);
    ...
    fn cnts_toss(
        ...
        mur: Noun,
        arms: &[(Noun, Noun)],
    ) -> Result<(u64, Vec<(Noun, Noun)>)> {
        let mut axis_match: Option<u64> = None;
        let mut out = Vec::with_capacity(arms.len());
        for (typ, foot) in arms {
            let (axis, new_typ) = self.cnts_tack(*typ, wing, mur)?;
    ...
    fn tack(&mut self, sut: Noun, hyp: &WingType, mur: Noun) -> Result<(u64, Noun)> {
    ...
    fn play_cnts(
        ...
        let port = self.cnts_base_port(sut, wing)?;
        let palo = match port {
            Port::Palo(palo) => palo,
            Port::Synthetic { typ, .. } => {
                if pairs.is_empty() {
    ...
    fn cnts_base_port(&mut self, sut: Noun, wing: &WingType) -> Result<Port> {
    ...
    fn garb_from_parts(&mut self, name: Option<&str>, poly: Poly, vair: Vair) -> Noun {
        let name_noun = match name {
            None => D(0),
            Some(term) => {
                let term_noun = term_to_noun(self.slab, term);
                T(self.slab, &[D(0), term_noun])
            }
        };
        ...
        T(self.slab, &[name_noun, poly_noun, vair_noun])
    }
    ...
    fn feel(&mut self, sut: Noun, wings: &[WingType]) -> Result<bool> {
        let mut current = sut;
    ...
    fn take<F>(&mut self, sut: Noun, vein: &[Option<BigUint>], duz: &F) -> Result<(u64, Noun)>
    where
        F: Fn(&mut Self, Noun) -> Result<Noun>,
    {
        let axis = tend(vein)?;
    ...
    fn take_inner<F>(&mut self, sut: Noun, vit: &[Option<u64>], duz: &F) -> Result<Noun>
    where
        F: Fn(&mut Self, Noun) -> Result<Noun>,
    {
            ...
            None => {
                ...
                match tag.as_str() {
                    "face" => {
                        ...
                        type_face_with_inner(self.slab, sut, new_inner)
                    }
                    "hint" => {
                        ...
                        Ok(ty_hint(self.slab, inner, note, new_payload))
                    }
                    "fork" => {
                        let options = type_fork_options(sut, &self.slab.noun_space())?;
                        let mut out = Vec::with_capacity(options.len());
                        for option in options {
                            out.push(self.take_inner_head_tail(option, None, tail, duz)?);
                        }
        ...
        F: Fn(&mut Self, Noun) -> Result<Noun>,
    {
        ...
        match tag.as_str() {
            ...
            "atom" => Ok(ty_void(self.slab)),
            "cell" => {
                let (head_ty, tail_ty) = type_cell_parts(sut, &self.slab.noun_space())?;
                if cap == 2 {
                    let new_head = self.take_axis(head_ty, mas, tail, duz, vil)?;
                    cell_type(self.slab, new_head, tail_ty)
                } else {
                    let new_tail = self.take_axis(tail_ty, mas, tail, duz, vil)?;
                    cell_type(self.slab, head_ty, new_tail)
                }
            }
            "core" => {
                if cap == 2 {
                    ...
                    Ok(ty_core(self.slab, new_payload, coil))
                }
            }
            "face" => {
                let inner = type_face_inner(sut, &self.slab.noun_space())?;
            ...
            "fork" => {
                ...
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.take_axis(option, step, tail, duz, vil)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
            ...
            _ => Err(CompilerError::UnsupportedExpr(format!(
                "native mint: take type {tag}"
            ))),
        }
    }

    fn gain(&mut self, sut: Noun, gen: &Hoon) -> Result<Noun> {
        self.chip(true, sut, gen)
    }

    fn lose(&mut self, sut: Noun, gen: &Hoon) -> Result<Noun> {
        ...
        match skin {
            ...
            Skin::Base(base) => match base {
                ...
                BaseType::Flag => {
                    ...
                    let yes_ty = self.gain_skin_inner(sut, ref_, &yes, seen)?;
                    let no_ty = self.gain_skin_inner(sut, ref_, &no, seen)?;
                    self.fork_from_options(vec![yes_ty, no_ty])
                }
                BaseType::Null => {
                ...
                BaseType::Atom(aura) => {
                    let mut seen_ref = StructNounSet::new();
            ...
            Skin::Help(help, inner) => {
                let payload = self.gain_skin_inner(sut, ref_, inner, seen)?;
    ...
    fn gain_atom_skin(
        ...
        match tag.as_str() {
            "void" => Ok(ty_void(self.slab)),
            "noun" => Ok(ty_atom(self.slab, aura, None)),
            "atom" => {
                ...
                let aura_str = atom_to_string(aura_atom)
                    .map_err(|err| CompilerError::Decode(format!("atom aura: {err}")))?;
                Ok(ty_atom(self.slab, &aura_str, ref_val))
            }
            "cell" => Ok(ty_void(self.slab)),
            "core" => Ok(ty_void(self.slab)),
            "face" => {
                let inner = type_face_inner(ref_, &self.slab.noun_space())?;
            ...
            "fork" => {
                ...
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.gain_atom_skin(sut, option, aura, seen)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
                let (inner, note, payload) = type_hint_parts(ref_, &self.slab.noun_space())?;
    ...
    fn gain_cell_skin(
        ...
        match tag.as_str() {
            "void" => Ok(ty_void(self.slab)),
            "noun" => {
                let head_ty = self.gain_skin_inner(sut, ref_, head, seen)?;
                if type_tag(head_ty, &self.slab.noun_space())? == "void" {
                    return Ok(ty_void(self.slab));
                }
            ...
            "cell" => {
                ...
                let head_ty = self.gain_skin_inner(sut, ref_head, head, seen)?;
                if type_tag(head_ty, &self.slab.noun_space())? == "void" {
                    return Ok(ty_void(self.slab));
                }
            ...
            "core" => {
                let (payload, coil) = type_core_parts(ref_, &self.slab.noun_space())?;
                let head_ty = self.gain_skin_inner(sut, payload, head, seen)?;
                if type_tag(head_ty, &self.slab.noun_space())? == "void" {
                    return Ok(ty_void(self.slab));
                }
                ...
                if matches!(tail, Skin::Base(BaseType::NounExpr)) {
                    Ok(ty_core(self.slab, head_ty, coil))
                } else {
                    let noun = ty_noun(self.slab);
            ...
            "fork" => {
                ...
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.gain_cell_skin(sut, option, head, tail, seen)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
    ...
    fn gain_leaf_skin(
        ...
        match tag.as_str() {
            ...
            "noun" => {
                let value = parsed_atom_to_noun(self.slab, atom);
                Ok(ty_atom(self.slab, aura, Some(value)))
            }
            "atom" => {
                let (ref_aura, ref_val) = type_atom_parts(ref_, &self.slab.noun_space())?;
                let value = parsed_atom_to_noun(self.slab, atom);
                ...
                let aura_str = atom_to_string(aura_atom)
                    .map_err(|err| CompilerError::Decode(format!("atom aura: {err}")))?;
                Ok(ty_atom(self.slab, &aura_str, Some(value)))
            }
            "cell" => Ok(ty_void(self.slab)),
            "core" => Ok(ty_void(self.slab)),
            "face" => {
            ...
            "fork" => {
                ...
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.gain_leaf_skin(sut, option, aura, atom, seen)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
    ...
    fn lose_skin(&mut self, sut: Noun, ref_: Noun, skin: &Skin) -> Result<Noun> {
        let mut seen = StructNounSet::new();
        ...
        match skin {
            ...
            Skin::Base(base) => match base {
                ...
                BaseType::Flag => {
                    let yes = Skin::Leaf("f".to_string(), ParsedAtom::Small(0));
                    let no = Skin::Leaf("f".to_string(), ParsedAtom::Small(1));
                    let without_yes = self.lose_skin_inner(sut, ref_, &yes, seen)?;
                ...
                BaseType::NounExpr => Ok(ty_void(self.slab)),
                BaseType::Atom(aura) => {
                    let mut seen_ref = StructNounSet::new();
    ...
    fn lose_atom_skin(
        ...
        match tag.as_str() {
            ...
            "noun" => {
                let noun = ty_noun(self.slab);
                Ok(ty_cell(self.slab, noun, noun))
            }
            "atom" => Ok(ty_void(self.slab)),
            "cell" => Ok(ref_),
            "core" => Ok(ref_),
            "face" => {
                ...
                type_face_with_inner(self.slab, ref_, inner)
            }
            "fork" => {
                let options = type_fork_options(ref_, &self.slab.noun_space())?;
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.lose_atom_skin(sut, option, _aura, seen)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
    ...
    fn lose_cell_skin(
        ...
        match tag.as_str() {
            ...
            "cell" => {
                let (ref_head, ref_tail) = type_cell_parts(ref_, &self.slab.noun_space())?;
                let lef = self.lose_skin_inner(sut, ref_head, head, seen)?;
                let rig = self.lose_skin_inner(sut, ref_tail, tail, seen)?;
                let cell_lr = cell_type(self.slab, lef, rig)?;
                let cell_l = cell_type(self.slab, lef, ref_tail)?;
                let cell_r = cell_type(self.slab, ref_head, rig)?;
                self.fork_from_options(vec![cell_lr, cell_l, cell_r])
            }
            "core" => {
                let (payload, coil) = type_core_parts(ref_, &self.slab.noun_space())?;
                let head_ty = self.lose_skin_inner(sut, payload, head, seen)?;
                if type_tag(head_ty, &self.slab.noun_space())? == "void" {
                    return Ok(ty_void(self.slab));
                }
            ...
            "fork" => {
                ...
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.lose_cell_skin(sut, option, head, tail, seen)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
    ...
    fn lose_leaf_skin(
        ...
        match tag.as_str() {
            ...
            "fork" => {
                ...
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.lose_leaf_skin(sut, option, _aura, atom, seen)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
    ...
    fn fuse(&mut self, sut: Noun, ref_: Noun) -> Result<Noun> {
        if let Some(cached) = self.fuse_boundary_lookup(sut, ref_)? {
            return Ok(cached);
        }
        // Structural recursion guard (matches hoon-138 `ut:fuse` using `bix` set).
        let mut seen: HashMap<(u32, u32), Vec<(Noun, Noun)>> = HashMap::new();
        let result = self.fuse_inner(sut, ref_, &mut seen)?;
        self.fuse_boundary_store(sut, ref_, result)?;
        Ok(result)
    }
        ...
        match tag.as_str() {
            "atom" => match type_tag(ref_, &self.slab.noun_space())?.as_str() {
                "atom" => {
                    ...
                    let aura_str = atom_to_string(aura_atom)
                        .map_err(|err| CompilerError::Decode(format!("atom aura: {err}")))?;
                    Ok(ty_atom(self.slab, &aura_str, value))
                }
                "cell" => Ok(ty_void(self.slab)),
            ...
            "cell" => match type_tag(ref_, &self.slab.noun_space())?.as_str() {
                "cell" => {
                    let (sut_head, sut_tail) = type_cell_parts(sut, &self.slab.noun_space())?;
                    let (ref_head, ref_tail) = type_cell_parts(ref_, &self.slab.noun_space())?;
                    let head = self.fuse_inner(sut_head, ref_head, seen)?;
                    let tail = self.fuse_inner(sut_tail, ref_tail, seen)?;
                    // Intersection of two cell types is empty if either component is empty.
                    // Without this, we can end up with `cell(void, ...)` which is not discarded
                    // from forks and breaks narrowing for tag tests like `?=([%i *] a)`.
                    if type_tag(head, &self.slab.noun_space())? == "void"
                        || type_tag(tail, &self.slab.noun_space())? == "void"
                    {
                        return Ok(ty_void(self.slab));
                    }
                    Ok(ty_cell(self.slab, head, tail))
            ...
            "face" => {
                ...
                type_face_with_inner(self.slab, sut, fused)
            }
            "fork" => {
                let options = type_fork_options(sut, &self.slab.noun_space())?;
                let mut out = Vec::with_capacity(options.len());
                for option in options {
                    out.push(self.fuse_inner(option, ref_, seen)?);
                }
                self.fork_from_options(out)
            }
            "hint" => {
                ...
                Ok(ty_hint(self.slab, inner, note, fused))
            }
            "hold" => {
    ...
    fn mull(&mut self, sut: Noun, gol: Noun, dox: Noun, gen: &Hoon) -> Result<(Noun, Noun)> {
        let gen_noun = self.hoon_noun_for_node(gen);
        if let Some(cached) = self.mull_cache_lookup(sut, gol, dox, gen_noun)? {
            return Ok(cached);
        }
        // hoon-138 pre-check: mull-none if sut is void
        if type_tag(sut, &self.slab.noun_space())?.as_str() == "void" {
    ...
    fn mull_inner(&mut self, sut: Noun, gol: Noun, dox: Noun, gen: &Hoon) -> Result<(Noun, Noun)> {
        match gen {
            // ---- Cell literal [^ *] ----
            Hoon::Pair(p, q) => {
                let noun_gol = ty_noun(self.slab);
                let hed = self.mull(sut, noun_gol, dox, p)?;
                let tal = self.mull(sut, noun_gol, dox, q)?;
                let p_ty = cell_type(self.slab, hed.0, tal.0)?;
                let p_ty = self.mull_nice(sut, gol, p_ty)?;
                let q_ty = cell_type(self.slab, hed.1, tal.1)?;
                Ok((p_ty, q_ty))
            }
            ...
            Hoon::DotKet(spec, q) => {
                let noun_gol = ty_noun(self.slab);
                let _q_result = self.mull(sut, noun_gol, dox, q)?;
            ...
            // ---- Increment: %dtls ----
            Hoon::DotLus(p) => {
                let atom_gol = ty_atom(self.slab, "$", None);
                let _p_result = self.mull(sut, atom_gol, dox, p)?;
                let atom_ty = ty_atom(self.slab, "$", None);
            ...
            Hoon::DotTar(p, q) => {
                let noun_gol = ty_noun(self.slab);
                let _p_result = self.mull(sut, noun_gol, dox, p)?;
                let _q_result = self.mull(sut, noun_gol, dox, q)?;
                let noun_ty = ty_noun(self.slab);
            ...
            Hoon::Tune(tune) => {
                let tool = term_or_tune_to_noun(self.slab, tune)?;
                let p_ty = ty_face_tool(self.slab, tool, sut);
                let q_ty = ty_face_tool(self.slab, tool, dox);
                Ok((p_ty, q_ty))
            }
            ...
            // ---- Hint note: %note ----
            Hoon::Note(note, inner) => {
                let vat = self.mull(sut, gol, dox, inner)?;
                let note_noun = note_to_noun(self.slab, note)?;
            ...
            // ---- Conditional: %wtcl ----
            Hoon::WutCol(p, q, r) => {
                let bool_gol = ty_bool(self.slab);
                let _nor = self.mull(sut, bool_gol, dox, p)?;

                // True branch: apply gain to both sut and dox
                let hiq: (Noun, Noun) = {
                    let fex_p = self.gain(sut, p)?;
                    let fex_q = self.gain(dox, p)?;
                    if type_tag(fex_p, &self.slab.noun_space())?.as_str() == "void" {
                        let q_ty = if type_tag(fex_q, &self.slab.noun_space())?.as_str() == "void" {
                            ty_void(self.slab)
                        } else {
                        ...
                        self.mull(fex_p, gol, fex_q, q)?
                    }
                };

                // False branch: apply lose to both sut and dox
                let ran: (Noun, Noun) = {
                    let wux_p = self.lose(sut, p)?;
                    let wux_q = self.lose(dox, p)?;
                    if type_tag(wux_p, &self.slab.noun_space())?.as_str() == "void" {
                        ...
                        self.mull(wux_p, gol, wux_q, r)?
                    }
                };

                // Fork both results
                let p_ty = self.fork_from_options(vec![hiq.0, ran.0])?;
                let p_ty = self.mull_nice(sut, gol, p_ty)?;
                let q_ty = self.fork_from_options(vec![hiq.1, ran.1])?;
                Ok((p_ty, q_ty))
            }
            ...
            Hoon::Fits(p, wing) => {
                ...
                let (_sut_ty, sut_formula) = self.mint(sut, noun_gol, &wing_hoon)?;
                let syx_p = self.cove(sut_formula)?;
                let (_dox_ty, dox_formula) = self.mint(dox, noun_gol, &wing_hoon)?;
                ...
                // Assert axes AND fish nock are identical
            ...
            Hoon::WutHax(_skin, wing) => {
                ...
                let (new_type, new_axis) = self.fend(sut, Way::Read, wing)?;
                let (old_type, old_axis) = self.fend(dox, Way::Read, wing)?;
            ...
            Hoon::ZapPat(wings, q, r) => {
                let feel_sut = self.feel(sut, wings)?;
                let feel_dox = self.feel(dox, wings)?;
    ...
    fn mull_nice(&mut self, _sut: Noun, gol: Noun, typ: Noun) -> Result<Noun> {
        if !self.vet {
            return Ok(typ);
        }
        if noun_eq(gol, ty_noun(self.slab), &self.slab.noun_space())?
    ...
    fn mull_beth(&mut self, sut: Noun, gol: Noun, typ: Noun) -> Result<(Noun, Noun)> {
        let p = self.mull_nice(sut, gol, typ)?;
        Ok((p, typ))
    }
    ...
    fn mull_grow(
        ...
        let noun_gol = ty_noun(self.slab);
        let dan = self.mull(sut, noun_gol, dox, ruf)?;
    ...
    fn mull_bake(
        ...
        self.with_stack_guard(|ut| {
            for (_arm_name, hoon) in arms {
                match hud {
                    Poly::Dry => {
                        let noun_gol = ty_noun(ut.slab);
                        let _ = ut.mull(sut, noun_gol, dox, hoon)?;
                    }
                    Poly::Wet => {
    ...
    fn mull_cnts(
        ...
        // find the wing from both perspectives
        let lug_p = self.find(sut, Way::Read, wing)?;
        let lug_q = self.find(dox, Way::Read, wing)?;
    ...
    fn mull_cnts_with_ports(
        ...
        match (lug_p, lug_q) {
            ...
            (Port::Synthetic { typ: typ_p, .. }, Port::Synthetic { typ: typ_q, .. }) => {
                ...
                let p_ty = self.mull_nice(sut, gol, *typ_p)?;
                Ok((p_ty, *typ_q))
            }
            ...
            (Port::Palo(palo_p), Port::Palo(palo_q)) => {
                let result = self.mull_endo(sut, gol, dox, palo_p, palo_q, pairs)?;
                // Apply nice check on sut-side result
                if self.vet {
                    let _ = self.mull_nice(sut, gol, result.0)?;
                }
                Ok(result)
            }
        }
    ...
    fn mull_endo(
        ...
        self.with_stack_guard(|ut| match (&palo_p.opal, &palo_q.opal) {
            ...
            (Opal::Leg(leg_p), Opal::Leg(leg_q)) => {
                let mut current_p = *leg_p;
                let mut current_q = *leg_q;
                for (sub_wing, expr) in rig {
                    let noun_gol = ty_noun(ut.slab);
                    let zil = ut.mull(sut, noun_gol, dox, expr)?;
...
fn garb_poly(garb: Noun, space: &NounSpace) -> Result<Poly> {
    let (_nym, poly, _vair) = garb_parts(garb, space)?;
    let atom = poly
        .in_space(space)
        .as_atom()
        .map_err(|err| CompilerError::Decode(format!("garb poly not atom: {err}")))?;
    let name =
        atom_to_string(atom).map_err(|err| CompilerError::Decode(format!("garb poly: {err}")))?;
    match name.as_str() {
        "wet" => Ok(Poly::Wet),
        "dry" => Ok(Poly::Dry),
        _ => Err(CompilerError::UnsupportedExpr(format!(
            "native mint: garb poly {name}"
        ))),
    }
}

fn garb_vair(garb: Noun, space: &NounSpace) -> Result<Vair> {
    let (_nym, _poly, vair) = garb_parts(garb, space)?;
    let atom = vair
        .in_space(space)
        .as_atom()
        .map_err(|err| CompilerError::Decode(format!("garb vair not atom: {err}")))?;
    let name =
        atom_to_string(atom).map_err(|err| CompilerError::Decode(format!("garb vair: {err}")))?;
    match name.as_str() {
        "gold" => Ok(Vair::Gold),
        "iron" => Ok(Vair::Iron),
        "lead" => Ok(Vair::Lead),
