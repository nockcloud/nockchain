//! §H adversarial audit — degree-adaptive FRI profile selection.
//!
...
//! (1) both degree classes hold the SAME 60-bit Johnson floor — there is no
//! weaker profile to grind toward; (2) the crossover is exactly at degree 15;
...
fn johnson_bits(c: &CircuitConfig) -> u32 {
    // PROD profiles use pow_bits = 0, so the Johnson floor is lb·nq.
    c.log_blowup * c.num_queries
}

#[test]
fn for_layer0_trace_boundary_floor_and_rounding() {
    // Both production profiles hold the identical 60-bit floor.
    assert_eq!(johnson_bits(&CircuitConfig::PROD), 60);
    assert_eq!(johnson_bits(&CircuitConfig::PROD_LB2_NQ30), 60);
    ...
    for bits in 1u32..=14 {
        ...
        assert_eq!(johnson_bits(&c), 60, "degree {bits} floor");
    }
